...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('entity_type', sa.String(20), nullable=False, comment='Type of entity: evidence or claim'),
        sa.Column('entity_id', postgresql.UUID(as_uuid=True), nullable=False, comment='ID of the entity being embedded'),
        sa.Column('embedding', Vector(384), nullable=False, comment='Vector embedding (384-dim for all-MiniLM-L6-v2)'),
        sa.Column('model_name', sa.String(100), nullable=False, server_default='all-MiniLM-L6-v2', comment='Name of the embedding model used'),
        sa.Column('model_version', sa.String(50), nullable=True, comment='Version of the embedding model'),
        sa.Column('tenant_id', sa.String(255), nullable=False, server_default='default', comment='Tenant ID for multi-tenancy isolation'),
    ...
    op.create_index('idx_embeddings_entity_unique', 'embeddings', ['entity_type', 'entity_id'], unique=True)
    ...
    op.create_table(
        'nli_results',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('claim_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('claims.id', ondelete='CASCADE'), nullable=False, comment='Claim being verified'),
        sa.Column('evidence_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('evidence.id', ondelete='CASCADE'), nullable=False, comment='Evidence used as premise'),
        sa.Column('label', sa.String(20), nullable=False, comment='NLI prediction: ENTAILMENT, CONTRADICTION, or NEUTRAL'),
        sa.Column('confidence', sa.Float(), nullable=False, comment='Probability of predicted label (0-1)'),
        sa.Column('entailment_score', sa.Float(), nullable=False, comment='Score for ENTAILMENT class'),
        sa.Column('contradiction_score', sa.Float(), nullable=False, comment='Score for CONTRADICTION class'),
        sa.Column('neutral_score', sa.Float(), nullable=False, comment='Score for NEUTRAL class'),
        sa.Column('model_name', sa.String(100), nullable=False, server_default='microsoft/deberta-v3-base', comment='NLI model used'),
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, server_default=sa.text('gen_random_uuid()')),
        sa.Column('claim_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('claims.id', ondelete='CASCADE'), nullable=False, comment='Claim being verified'),
        sa.Column('verdict', sa.String(20), nullable=False, comment='Final verdict: SUPPORTED, REFUTED, or INSUFFICIENT'),
        ...
        sa.Column('refute_score', sa.Float(), nullable=False, comment='Weighted contradiction score'),
        sa.Column('neutral_score', sa.Float(), nullable=False, comment='Weighted neutral score'),
        sa.Column('evidence_count', sa.Integer(), nullable=False, comment='Number of evidence items analyzed'),
        sa.Column('supporting_evidence_count', sa.Integer(), nullable=False, server_default='0', comment='Count of supporting evidence'),
        sa.Column('refuting_evidence_count', sa.Integer(), nullable=False, server_default='0', comment='Count of refuting evidence'),
        sa.Column('neutral_evidence_count', sa.Integer(), nullable=False, server_default='0', comment='Count of neutral evidence'),
        sa.Column('reasoning', sa.Text(), nullable=True, comment='Human-readable explanation of the verdict'),
        sa.Column('nli_result_ids', postgresql.ARRAY(postgresql.UUID(as_uuid=True)), nullable=True, comment='Array of NLI result IDs used'),
        sa.Column('pipeline_version', sa.String(50), nullable=True, comment='Version of the verification pipeline'),
        sa.Column('retrieval_method', sa.String(50), nullable=True, comment='Retrieval method: vector, hybrid, or keyword'),
        sa.Column('created_at', sa.DateTime(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.Column('updated_at', sa.DateTime(), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
    ...
    op.create_index('idx_verification_results_confidence', 'verification_results', [sa.text('confidence DESC')])
    op.create_index('idx_verification_results_created_at', 'verification_results', [sa.text('created_at DESC')])
